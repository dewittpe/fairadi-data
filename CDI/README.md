...
```sh
make -C CDI total_population_and_housing_units.csv.gz
make -C CDI component01.csv.gz
make -C CDI component09.csv.gz
make -C CDI faircdi.csv.gz
```
...
```r
DT[, componentXX := scale(componentXX), by = .(year)]
```
...
- "bad" components are expected to increase with deprivation
- "good" components are protective / affluence measures
...
- geography identifiers: `year`, `state`, `county`, `tract`, `block_group`
- standardized components: `component01` through `component18`
- `cdiraw`
- `cdistd`
- `faircdi`

...
neighborhood_atlas <-
  list(
    "2020" = data.table::fread("../../adi/neighborhood-atlas/neighborhood-atlas-2020-adi-v4_0_1.csv", colClasses = "character"),
    "2023" = data.table::fread("../../adi/neighborhood-atlas/neighborhood-atlas-2023-adi-v4_0_1.csv", colClasses = "character")
  ) |>
...
neighborhood_atlas[, neighborhood_atlas_exclude := data.table::fifelse(ADI_STATERNK %in% as.character(1:10), "", ADI_STATERNK)]
...
adi <- merge(
  x = adi, #[year %in% unique(neighborhood_atlas$year)],
  y = neighborhood_atlas,
  all = TRUE,
  by = c("year", "FIPS")
)

adi[year %in% c(2020, 2023), .N, keyby = .(year, exclude_reason, neighborhood_atlas_exclude)] |>
  print(nrow = Inf)

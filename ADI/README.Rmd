...
atlas_requirements <-
  data.table::data.table(
    envvar = c(
      "NEIGHBORHOOD_ATLAS_ADI_2020_V401",
      "NEIGHBORHOOD_ATLAS_ADI_2023_V401"
    ),
    label = c("2020 Area Deprivation Index v4.0.1", "2023 Area Deprivation Index v4.0.1"),
    sha256 = c(
...
atlas_basename <- function(envvar) {
  path <- Sys.getenv(envvar, unset = "")
  if (!nzchar(path)) {
    return(sprintf("<unset: %s>", envvar))
  }
  basename(path)
}

atlas_preflight <- function(requirements = atlas_requirements) {
  result <- data.table::copy(requirements)
  result[, path := Sys.getenv(envvar, unset = "")]
  result[, is_set := nzchar(path)]
  result[, exists := is_set & file.exists(path)]
  result[, actual_sha256 := NA_character_]
  result[exists == TRUE, actual_sha256 := vapply(
    path[exists == TRUE],
    digest::digest,
    character(1),
    algo = "sha256",
  ...
  result[, sha_ok := exists & !is.na(actual_sha256) & actual_sha256 == sha256]
  result[
    ,
    status := data.table::fcase(
      !is_set, "path not defined",
      !exists, "file not found",
      !sha_ok, "sha256 mismatch",
      default = "ok"
    )
  ]
  list(ok = all(result$status == "ok"), details = result)
}
```
...
- `r atlas_basename("NEIGHBORHOOD_ATLAS_ADI_2020_V401")`
...
- `r atlas_basename("NEIGHBORHOOD_ATLAS_ADI_2023_V401")`
...
```{r check-for-neighborhood-atlas-files, echo = FALSE, results = "asis"}
atlas_status <- atlas_preflight()

if (!atlas_status$ok) {
  cat(
    "Neighborhood Atlas diagnostics were not run because the required file",
    "paths were not configured or did not pass SHA256 verification.\n\n"
  )
  cat("Set the following environment variables to the expected files and rebuild:\n\n")

  for (i in seq_len(nrow(atlas_status$details))) {
    row <- atlas_status$details[i]
    cat(
      sprintf(
        "- `%s` (%s): %s",
        row$envvar,
    ...
    if (!is.na(row$actual_sha256) && row$status == "sha256 mismatch") {
      cat(sprintf(" (expected `%s`, got `%s`)", row$sha256, row$actual_sha256))
    }
    cat("\n")
  }

  cat(
    "\nThe README was knitted up to this point and then exited intentionally.\n"
  )
  knitr::knit_exit()
}
```
Import the Neighborhood Atlas 2020 and 2023 data.
```{r import-neighborhood-atlas, include = FALSE}
neighborhood_atlas <-
  list(
...
```{r eda, include = FALSE}
print(fairadi[year <= 2012, .N, keyby = .(year, exclude_reason)], nrow = Inf)
```

```{r merge-fairadi-neighborhood-atlas, include = FALSE}
# In this README we will subset to the two years of Neighborhood Atlas data.
fairadi <- subset(fairadi, year %in% c(2020, 2023))
deltas <- merge(x = fairadi, y = neighborhood_atlas, all = TRUE, by = c("year", "FIPS"))
```
...
```{r exlcusion-criterial, echo = FALSE, results = "asis"}
x <- deltas[, .N, keyby = .(year, f = exclude_from_ranking, n = neighborhood_atlas_exclude)]
x[, f := factor(f, c(0, 1), c("Include", "Exclude"))]
x[, n := factor(n, c(0, 1), c("Include", "Exclude"))]
kableExtra::kbl(
  x = x[, -"year"],
  col.names = c("fairadi", "Neighborhood Atlas", "Number of Block Groups")
) |>
kableExtra::pack_rows(index = table(x$year))
```
...
bggq <-
  deltas[
    exclude_from_ranking == 0 & neighborhood_atlas_exclude == 1,
    qwraps2::n_perc(neighborhood_atlas_exclude_reason == "GQ", show_denom =
      "always")
  ]
```
The primary reason for exclusion by Neighborhood Atlas is group quarters, `r bggq`.
...
```{r, include = FALSE}
stopifnot(
  deltas[neighborhood_atlas_exclude == "GQ", .SD[duplicated(.SD, by = c("state", "county", "tract", "block_group", "neighborhood_atlas_exclude_reason"))]][, .N == 0]
)
```

There are many ways the reason for exclusion will change from 2020 to 2023
within the Neighborhood Atlas data.
```{r neighborhood-atlas-exclude-reasons, echo = FALSE, fig.cap = "Block group exclusion reason by year for Neighborhood Atlas data.  GQ: group quarters, PH: total population, QDI: questionable data quality, blank: included in that years rankings."}
x <-
  data.table::dcast(
    neighborhood_atlas[
...
ggplot2::ggplot(x) +
ggplot2::aes(x = `2020`, y = `2023`, fill = N, label = N) +
ggplot2::scale_fill_viridis_c(name = "Block Groups") +
ggplot2::geom_tile() +
...
deltas <- merge(deltas, data.table::fread("../FIPS/2020__states.csv"), by = "state")
deltas[, adi_raw := NULL]
deltas[, county := NULL]
deltas[, tract := NULL]
...
```{r abs-rank-delta-national, echo = FALSE, fig.cap = cap}
abs_rank_delta_plot(deltas_long[startsWith(variable, "within") & Level == "national"]) +
ggplot2::scale_x_continuous(breaks = seq(0, 12, by = 2))
cap <-
"Cumulative percentage of block groups with differences in national percentile ranking between fairadi and Neighborhood Atlas.  Each black line is an individual state/territory, and the red line with dots is for the whole dataset."
```

```{r abs-rank-delta-state, echo = FALSE, fig.cap = cap}
abs_rank_delta_plot(deltas_long[startsWith(variable, "within") & Level == "state"]) +
ggplot2::scale_x_continuous(breaks = seq(0, 5, by = 1))
"Cumulative percentage of block groups with differences in state decile ranking between fairadi and Neighborhood Atlas.  Each black line is an individual state/territory, and the red line with dots is for the whole dataset."
```
...
```{r corplot, echo = FALSE, fig.cap = cap}
...
cap <- "Pearson correlation between each of the ADI topics and the fairadi National Percentile Ranking"
```
